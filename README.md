...
- **Refresh-токен** — случайный идентификатор, хранится в **Redis** (хэш SHA-256, не в
  открытом виде) с TTL и ротируется при каждом `/auth/refresh`.
- **Reuse detection:** если приходит уже использованный (ротированный) refresh-токен,
  это считается кражей — отзываются **все** сессии пользователя, нужно залогиниться заново.
- **`/auth/logout` требует access-токен** и удаляет refresh только если он принадлежит
  этому же пользователю — зная чужой refresh-токен, разлогинить его нельзя.
